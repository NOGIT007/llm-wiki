...
import { resolveVault, sanitizeVaultName, loadVault, vaultPages, hiddenVaults, saveHiddenVaults, getPages, reloadAllVaults, listVaults } from "./src/vaults";
...
async function handleArchitecture(): Promise<Response> {
...
async function handleVaults(req: Request): Promise<Response> {
  if (req.method === "GET") {
...
async function handleVaultByName(req: Request): Promise<Response> {
  if (req.method !== "DELETE") return new Response("Method not allowed", { status: 405 });
  const rawName = new URL(req.url).pathname.replace("/api/vaults/", "");
...
async function handleGraph(req: Request): Promise<Response> {
  const { pages } = await getVaultContext(req);
  return Response.json(buildGraphData(pages), { headers: NO_CACHE });
}

async function handlePages(req: Request): Promise<Response> {
  const { pages } = await getVaultContext(req);
...
async function handlePage(req: Request): Promise<Response> {
  const { pages } = await getVaultContext(req);
  const slug = new URL(req.url).pathname.replace("/api/page/", "");
...
async function handleSearch(req: Request): Promise<Response> {
  const { url, pages } = await getVaultContext(req);
  const q = url.searchParams.get("q") || "";
  return Response.json(searchPages(q, pages), { headers: NO_CACHE });
}

async function handleWikiHealth(req: Request): Promise<Response> {
  const { vault, pages } = await getVaultContext(req);
...
async function handleRaw(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const vaultParam = url.searchParams.get("vault");
  const vault = resolveVault(vaultParam);
...
async function handleChatModels(): Promise<Response> {
  let config: any = {};
  try { config = JSON.parse(await readFile(CONFIG_FILE, "utf-8")); } catch {}
...
async function handleBackupStatus(): Promise<Response> {
...
// ── Server with declarative routes ──
...
const server = Bun.serve({
  ...
  routes: {
    ...
    "/api/reload": handleReload,
    "/api/vaults": handleVaults,
    "/api/vaults/:name": handleVaultByName,
    "/api/graph": handleGraph,
    "/api/pages": handlePages,
    "/api/page/:slug": handlePage,
    "/api/search": handleSearch,
    "/api/wiki/health": handleWikiHealth,
    "/api/config": handleConfig,
    "/api/queue": handleQueue,
    "/api/raw": handleRaw,
    "/api/chat/models": handleChatModels,
    "/api/chat": handleChat,
    "/api/backup/status": handleBackupStatus,
    "/api/backup": handleBackup,
  },
  async fetch(req) {
    // Static asset fallback for JS/CSS/SVG files
    const url = new URL(req.url);
